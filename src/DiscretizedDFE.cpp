...
void
DiscretizedDFE::CreateCumulativeProbs () {
		cumProbs[0] = _pClassProbabilities[0];
    for(int i = 1; i< _pClassProbabilities.size(); i++)
        {
...
void
DiscretizedDFE::SetProb(const dvector& probs, const double mutationRate)
{
...
int
DiscretizedDFE::AssignFitnessToBin (double W) const
{
    if (W < min) {
        Rcpp::stop("Can't assign below the min!");
    }
    W = W * ANCESTRALGROWTHRATE;
    double dif = abs(W - MidPoints[0]);
...
void
DiscretizedDFE::UpdateWithNewSamples (const std::vector<ObservedWell>& AugmentedData)
{
    ...
    double totalTime = std::accumulate(AugmentedData.begin(), AugmentedData.end(), 0.0,
    	[](double b, const ObservedWell& ow) {
...
void
DiscretizedDFE::SetDFEasPointMass (int IndexToSet)
{
